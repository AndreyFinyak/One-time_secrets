...
def log_action(db: Session, action: str, secret_id: int = None, ip: str = None, metadata: dict = None):
    ...
    if not db.is_active:
        ...
        # return # Раскомментируйте, если хотите пропустить лог при неактивной сессии
        # Или попробуйте получить новую сессию, если это возможно в вашем контексте
        # from .database import SessionLocal
        # db = SessionLocal() # Это может быть не лучшим решением здесь
...
def create_secret(db: Session, secret: schemas.SecretCreate, ip: str = None):
    ...
    db_secret = models.Secret(
        ...
        passphrase=secret.passphrase, # Убедитесь, что пароль хешируется, если он используется для аутентификации
